...
cor_dir<-"Z:/scratch/TOPMed_Proteome/10INTERVAL_prediction/double_dipped_correlation/"
...
complete_correlations<-bind_rows(cor_list) %>%
  mutate(train_pop=substr(source,1,3),
         PAV_filtered=if_else(grepl("PAV_filtered",source),"TRUE","FALSE"),
...
for (m in 1:length(model_vec)){
  ...
  tmp<-complete_correlations %>% filter(model==model_vec[m],PAV_filtered=="FALSE")
...
anova_list<-vector(mode="list",length=length(model_vec))
tukey_list<-vector(mode="list",length=length(model_vec))
gene_list_PAV_filtered<-vector(mode="list",length=length(model_vec))
F_permuted_list<-vector(mode="list",length=length(model_vec))

for (m in 1:length(model_vec)){
  #grab the intersection of proteins present in all training pops
  tmp<-complete_correlations %>% filter(model==model_vec[m],PAV_filtered=="TRUE")
  ...
  tmp_ubiquitous <- tmp %>% filter(gene_id %in% ubiquitous_proteins)
  #store the intersection for plotting later
  gene_list_PAV_filtered[[model_vec[m]]]<-tmp_ubiquitous
...
fwrite(bind_rows(anova_list),"Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_pop_ANOVA_summaries_PAV_filtered.csv")
fwrite(bind_rows(tukey_list),"Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_pop_TUKEY_summaries_PAV_filtered.csv")
```
...
gene_subset<-bind_rows(gene_list_PAV_filtered)
...
gene_subset %>% 
  ...
  theme_bw() +
  facet_wrap(~model,ncol=1) +
  scale_fill_viridis(discrete=T)  + theme(legend.position = "none") + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_compare_pops_PAV_filtered.pdf",dpi=700) + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_compare_pops_PAV_filtered.png",dpi=700) + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_compare_pops_PAV_filtered.tiff",dpi=700)
...
ggplot(data=gene_subset,aes(x=train_pop,y=estimate)) +
  ...
  theme_bw() +
  facet_wrap(~model,ncol=1) +
  scale_fill_viridis(discrete=T)  + theme(legend.position = "none") + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/supplemental_INTERVAL_compare_pops_PAV_filtered.pdf",dpi=700,height=7,width=7) + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/supplemental_INTERVAL_compare_pops_PAV_filtered.png",dpi=700,height=7,width=7) + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/supplemental_INTERVAL_compare_pops_PAV_filtered.tiff",dpi=700,height=7,width=7)
