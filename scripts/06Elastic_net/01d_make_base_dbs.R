...
for (pop in populations){
  out_prefix<-'/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/dbs_out/' %&% pop %&% '_PCAIR_baseline_models'
  
  model_summaries <- read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr1_base_chr1_model_summaries.txt', header = T, stringsAsFactors = F)
  tiss_summary <- read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr1_base_chr1_tiss_chr_summary.txt', header = T, stringsAsFactors = F)
  weights <- read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr1_base_chr1_weights.txt', header = T, stringsAsFactors = F)
  covariances<-read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr1_base_chr1_covariances.txt', header = T, stringsAsFactors = F)
  n_samples <- tiss_summary$n_samples
  
  cat("Reading in results per chr \n")
  for ( chr in c(2:22)){
    cat(chr,"\n")
    tmp <- read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr' %&% chr %&% '_base_chr' %&% chr %&% '_model_summaries.txt', header = T, stringsAsFactors = F)
    model_summaries<-rbind.data.frame(model_summaries,tmp)
    tmp2 <- read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr' %&% chr %&% '_base_chr' %&% chr %&% '_tiss_chr_summary.txt', header = T, stringsAsFactors = F)
    tiss_summary<-rbind.data.frame(tiss_summary,tmp2)
    tmp3 <- read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr' %&% chr %&% '_base_chr' %&% chr %&% '_weights.txt', header = T, stringsAsFactors = F)
    weights<-rbind.data.frame(weights,tmp3)
    tmp4<-read.table('/home/rschubert1/data/TOPMED_Proteome/' %&% pop %&% '/PCAIR_modeling/06Elastic_net/base_out/' %&% pop %&% '_chr' %&% chr %&% '_base_chr' %&% chr %&% '_covariances.txt', header = T, stringsAsFactors = F)
