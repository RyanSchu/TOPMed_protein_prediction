...
```{r}
dir<-"Z:/scratch/TOPMed_Proteome/10INTERVAL_prediction/double_dipped_correlation/"
...
```{r}
dir<-"Z:/scratch/TOPMed_Proteome/10INTERVAL_prediction/PAV_adj_correlations/"
...
adj_list<-fread("Z:/scratch/TOPMed_Proteome/06Elastic_net/PAV_modeling/adj_protein_list.csv") %>% 
  ...
  rename_with(.fn=function(x){gsub("PAV_adj_","",x)})

compare_methods<-full_join(base_correlations,PAV_correlations,by=c("gene_id","train_pop")) %>% inner_join(adj_list,by=c("gene_id"="gene","train_pop"))
