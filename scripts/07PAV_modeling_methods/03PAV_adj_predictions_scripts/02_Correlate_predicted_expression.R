...
##################################################################################################
predicted_dir<-"/home/rschubert1/scratch/TOPMed_Proteome/10INTERVAL_prediction/PAV_adj_predictions/"
file_list<-list.files(path=predicted_dir,pattern="*prediction.txt")
...
for (i in 1:length(file_list))  {#How well does each model replicate
    ...
    ##################################################################################################
    out_tag<-gsub("prediction.txt","",file_list[i])
    fwrite(predictive_correlations,"/home/rschubert1/scratch/TOPMed_Proteome/10INTERVAL_prediction/PAV_adj_correlations/" %&% out_tag %&% "correlation.txt",col.names = T,sep='\t')
}#close
