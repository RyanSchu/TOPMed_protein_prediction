...
```{r cars}
pop_list<-c("AFA","CHN","HIS","CAU","ALL")


comparison_list<-list()
for (pop in pop_list){
  files<-list.files(path="Z:/scratch/TOPMed_Proteome/10INTERVAL_prediction/double_dipped_correlation/",pattern=pop)
  files<-files[!grepl("PAV_filtered",files)]
  file_list<-list(rep(NA,length(files)))
  for (i in 1:length(files)){
    file_list[[i]]<-fread("Z:/scratch/TOPMed_Proteome/10INTERVAL_prediction/double_dipped_correlation/" %&% files[i]) %>% mutate(model=files[i])
  }
...
signif_comparisons<-complete_comparisons %>% filter(wilcox.p.val < 0.05)
```

## then handle the pav filtered
...
fwrite(complete_comparisons,"Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_statistical_tests_comparison_to_baseline_PAV_filtered.csv",sep=',',col.names = T)
signif_comparisons_PAV_filtered<-complete_comparisons %>% filter(t.p.val < 0.05)
```