...
##################################################################################################
predicted_dir<-"/home/rschubert1/scratch/TOPMed_Proteome/10INTERVAL_prediction/true_subset_predictions/"
...
observed_expression<-fread("zcat /data/rschubert1/TOPMED_Proteome/INTERVAL/protein_expr/renamed_protein_expr_raw.txt.gz", header = T, sep = '\t',stringsAsFactors = F) %>%
...
for (i in 1:length(file_list))  {#How well does each model replicate
    ...
    fwrite(predictive_correlations,"/home/rschubert1/scratch/TOPMed_Proteome/10INTERVAL_prediction/true_subset_correlations/" %&% out_tag %&% "correlation.txt",col.names = T,sep='\t')
}#close
