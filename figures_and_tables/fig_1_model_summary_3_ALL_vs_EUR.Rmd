...
for(pop in c("ALL", "AFA", "CHN", "EUR", "HIS")){
  bl <- fread("ryans-results/rho_avg_values_" %&% pop %&% "_PCAIR_baseline_models.txt") %>% mutate(pop=pop, model="baseline")
  if(exists('res')){
    res <- rbind(res, bl)
  }else{
    res <- bl
  }
  fm <- fread("ryans-results/rho_avg_values_" %&% pop %&% "_PCAIR_dapg_0.001_T.txt") %>% mutate(pop=pop, model="fine-mapped")
    res <- rbind(res, fm)
}
...
counts <- fread('protein_counts.txt')
...
png("Fig1.png", width = 19, height =8, units = 'cm', res = 300)
fig1
dev.off()
```
...
```{r}
for(pop in c("ALL", "AFA", "CHN", "EUR", "HIS")){
  bl <- fread("ryans-results/" %&% pop %&% "_PCAIR_baseline_models_rho0.1_zpval0.05_correlation.txt") %>% mutate(pop=pop, model="baseline")
  fm <- fread("ryans-results/" %&% pop %&% "_PCAIR_dapg_0.001_T_rho0.1_zpval0.05_correlation.txt") %>% mutate(pop=pop, model="fine-mapped")
...
```{r}
pop="ALL"
allbl <- fread("ryans-results/" %&% pop %&% "_PCAIR_baseline_models_rho0.1_zpval0.05_correlation.txt") %>% mutate(pop=pop, model="baseline")
allfm <- fread("ryans-results/" %&% pop %&% "_PCAIR_dapg_0.001_T_rho0.1_zpval0.05_correlation.txt") %>% mutate(pop=pop, model="fine-mapped")
dim(allbl)
dim(allfm)
pop="EUR"
eurbl <- fread("ryans-results/" %&% pop %&% "_PCAIR_baseline_models_rho0.1_zpval0.05_correlation.txt") %>% mutate(pop=pop, model="baseline")
eurfm <- fread("ryans-results/" %&% pop %&% "_PCAIR_dapg_0.001_T_rho0.1_zpval0.05_correlation.txt") %>% mutate(pop=pop, model="fine-mapped")
dim(eurbl)
dim(eurfm)
#all pop res
...
print(fig3)
tiff("Fig3.tiff", width = 19, height = 9.5, units = 'cm', res = 300, compression = 'lzw')
fig3
dev.off()
png("Fig3.png", width = 19, height = 9.5, units = 'cm', res = 300)
fig3
dev.off()
...
fig3alt<- ggplot(blfm, aes(x=estimate.y, y=estimate.x)) + geom_point() + facet_wrap(~model.x) + geom_abline(slope=1,intercept=0) + geom_density_2d() +
  ...
  ggtitle("B")
print(fig3alt)
tiff("Fig3alt.tiff", width = 19, height = 9.5, units = 'cm', res = 300, compression = 'lzw')
fig3alt
dev.off()
png("Fig3alt.png", width = 19, height = 9.5, units = 'cm', res = 300)
fig3alt
dev.off()
```
...
print(fig3ab)
tiff("Fig3ab.tiff", width = 18, height = 19, units = 'cm', res = 300, compression = 'lzw')
fig3ab
dev.off()
png("Fig3ab.png", width = 18, height = 19, units = 'cm', res = 300)
fig3ab
dev.off()
```
