...
get_aptamer_ensemble_annotation<-function(aptamer_ensemble_annotation_file_name){ #read in the mapping of aptamers to ensemble ids
  read.table(aptamer_ensemble_annotation_file_name,sep=",",quote="\"",header=T)
}

get_PAV_mapping<-function(PAV_mapping_file_name){ #gets the parsed output of the ensemble vep tool
  PAV_anno<-read.table(PAV_mapping_file_name,header=F)
  colnames(PAV_anno)<-c("snp","geneId","transcriptId","gene_type","consequence")
  return(PAV_anno)
}

extract_PAV_genotypes<-function(genotype,modelId,aptENSMapping,PAVmapping){
  Soma<-strsplit(modelId,split="_")[[1]][1]
  ENSGId<-strsplit(modelId,split="_")[[1]][2] %>% gsub("\\.[0-9]+","",.)
  transcripts<-filter(aptENSMapping,SomaId == Soma,gene==ENSGId) %>% select(transcript) %>% unlist() %>% unname()
  PAVs<-PAVmapping %>% filter(geneId == ENSGId, transcriptId %in% transcripts) %>% select(snp) %>% unlist() %>% unname()
  PAVgenotypes<-genotype[,colnames(genotype) %in% PAVs] 
  print(sum(colnames(genotype) %in% PAVs))
  if(sum(colnames(genotype) %in% PAVs) == 0) {return(NA)}
  return(list(PAV=PAVgenotypes,npav=sum(colnames(genotype) %in% PAVs)))
}
  
remove_PAV_genotypes<-function(genotype,modelId,aptENSMapping,PAVmapping){
  Soma<-strsplit(modelId,split="_")[[1]][1]
  ENSGId<-strsplit(modelId,split="_")[[1]][2] %>% gsub("\\.[0-9]+","",.)
  transcripts<-filter(aptENSMapping,SomaId == Soma,gene==ENSGId) %>% select(transcript) %>% unlist() %>% unname()
  PAVs<-PAVmapping %>% filter(geneId == ENSGId, transcriptId %in% transcripts) %>% select(snp) %>% unlist() %>% unname()
...
calculate_pca_scores<-function(PAVgenotypes,minCumVar=0.90){
  pca<-princomp(PAVgenotypes)
  scores<-pca$scores[,1:which(cumsum(pca$sdev^2) / sum(pca$sdev^2) > minCumVar)[1]] #select pcs which meet a minimum cumulative variance
  return(scores)
}
  
adjust_protein_expression_by_PAVscores<-function(expression_vec, PAVscores){
  combined_df <- cbind.data.frame(expression_vec, PAVscores)
  expr_resid <- summary(lm(expression_vec ~ ., data=combined_df))$residuals
  expr_resid <- scale(expr_resid, center = TRUE, scale = TRUE)
  expr_resid
}
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 ...
                 PAV_mapping_file,aptamer_ensemble_annotation_file) {
  ...
  PAVmap<-get_PAV_mapping(PAV_mapping_file)
  aptamerENSmap<-get_aptamer_ensemble_annotation(aptamer_ensemble_annotation_file)
  # Set seed----
  ...
  model_summary_cols <- c('gene_id', 'gene_name', 'gene_type', 'alpha', 'n_snps_in_window','n_PAVs_found' ,'n_snps_in_model', 'lambda_min_mse',
  ...
  for (i in 1:n_genes) {
    ...
    PAV_data <- extract_PAV_genotypes(cis_gt,gene,aptamerENSmap,PAVmap)
    
    # print(str(PAV_gt))
    cis_gt <- remove_PAV_genotypes(cis_gt,gene,aptamerENSmap,PAVmap)
    if (all(is.na(cis_gt))) {
    ...
    model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), n_PAV,0, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      if(!is.na(covariates_df)){
        # print("adjusting by covariates")
	adj_expression <- adjust_for_covariates(expression_vec, covariates_df)
      } else if (n_PAV != 0){
	# print("adjusting by PAV PCA")
        # print(str(PAV_gt))
        scores<-calculate_pca_scores(PAV_gt)
        # print(str(scores))
        adj_expression <- adjust_protein_expression_by_PAVscores(expression_vec,scores) %>% as.matrix()
      } else {
	# print("leaving unadjusted")
      ...
      if (length(fit) > 0) {
        ...
        if (fit$nzero[best_lam_ind] > 0) {
          ...
          model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), n_PAV ,fit$nzero[best_lam_ind], fit$lambda[best_lam_ind], R2_avg, R2_sd,
                             cv_R2_avg, cv_R2_sd, training_R2, pval_est, rho_avg, rho_se, rho_zscore, rho_avg_squared, zscore_pval,
                             cv_rho_avg, cv_rho_se, cv_rho_avg_squared, cv_zscore_est, cv_zscore_pval, cv_pval_est)
        } else {
          model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), n_PAV,0, fit$lambda[best_lam_ind], R2_avg, R2_sd,
                             cv_R2_avg, cv_R2_sd, training_R2, pval_est, rho_avg, rho_se, rho_zscore, rho_avg_squared, zscore_pval,
                             cv_rho_avg, cv_rho_se, cv_rho_avg_squared, cv_zscore_est, cv_zscore_pval, cv_pval_est)
        }
      } else {
        model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), n_PAV,0, NA, R2_avg, R2_sd, 
                           NA, NA, NA, pval_est, rho_avg, rho_se, rho_zscore, rho_avg_squared, zscore_pval,
                           NA, NA, NA, NA, NA, NA)
      }
    }
    write(model_summary, file = model_summary_file, append = TRUE, ncol = 25, sep = '\t')
