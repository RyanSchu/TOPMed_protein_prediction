...
for (pop in pops){
  ...
  base_pav<-fread(cor_dir %&% pop %&% "_PCAIR_baseline_models_PAV_filtered_rho0.1_zpval0.05_correlation.txt") %>% 
    rename_with(.fn=function(x){return(paste(x,"base",sep="_"))})
  ...
  for (f in 1:length(files))  {
    ...
    if( grepl("PAV_filtered",files[f])){
      compare_list[[f]]<-inner_join(base_pav,tmp,by=c("gene_id_base"="gene_id_fm"))
    } else {
...
complete_comparisons<-bind_rows(pop_list) %>% 
  mutate(train_pop=substr(fm_source,1,3),
         PAV_filtered=if_else(grepl("PAV_filtered",fm_source),"TRUE","FALSE"),
...
complete_comparisons %>%
  filter(model %in% c("0.001_T"),PAV_filtered == "FALSE") %>% 
...
complete_comparisons %>%
  filter(model %in% c("0.001_T"),PAV_filtered == "TRUE") %>% 
  ggplot(data=.,aes(x=estimate_base,y=estimate_fm)) +
  theme_bw() +
  geom_abline(slope=1,intercept = 0) +
  geom_point() +
  ...
  geom_smooth(method="lm",se=F) +
  geom_density_2d(alpha=0.5) +
  ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_scatter_plot_PAV_filtered.pdf",dpi=700,width=7.29,height=4.5)  + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_scatter_plot_PAV_filtered.png",dpi=700,width=7.29,height=4.5)  + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/INTERVAL_scatter_plot_PAV_filtered.tiff",dpi=700,width=7.29,height=4.5)   
...
complete_comparisons %>%
  ...
  geom_smooth(method="lm",se=F) +
  geom_density_2d(alpha=0.5) +
  ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/supplemental_INTERVAL_scatter_grid_PAV_filtered.pdf",dpi=700,width=7.29,height=4.5)  + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/supplemental_INTERVAL_scatter_grid_PAV_filtered.png",dpi=700,width=7.29,height=4.5)  + ggsave("Z:/scratch/TOPMed_Proteome/figures_and_tables/final_figures_and_tables/figures_and_tables/supplemental_INTERVAL_scatter_grid_PAV_filtered.tiff",dpi=700,width=7.29,height=4.5)   
...
all_estimates %>% 
  filter(model %in% c("0.001_T","baseline"),PAV_filtered==FALSE) %>%
...
all_estimates %>% 
  filter(model %in% c("0.001_T","baseline"),PAV_filtered==T) %>%
  separate(gene_id,into=c("apt","ENSG"),sep="_") %>%
  select(apt) %>%
  arrange(apt) %>%
  distinct %>% unlist() %>% unname() %>% length()
